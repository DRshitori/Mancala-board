...
font = pygame.font.Font('assets/arial.ttf', 12)
...
class Drawer:
    ...
    def PlayerScore(self, Player, value):
        if (Player == 1):
            text = font.render(f"Player's score : {value}", True, (84, 180, 53), board_color)
            textRect = text.get_rect()
            textRect.center = WIDTH  - 80, magasin_height + 300
        else:
            text = font.render(f"AI's score : {value}", True, (69, 153, 186), board_color)
            textRect = text.get_rect()
    ...
    def DisplayTurn(self, Player):
        font = pygame.font.Font('./assets/arial.ttf', 15)
        if (Player == 2):
